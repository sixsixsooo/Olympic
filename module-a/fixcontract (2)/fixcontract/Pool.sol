...
contract Pool {
  ...
  //Функция для вывода стоимости второго токена в eth возращает стоимость второго токена
    ...
    //Функция для обмена первого токена на второй, принимает количество первого токена  возвращает количество второго токена которое нужно получить
        function tradeFirstToSecond(uint256 firstAmount, address sender) public returns(uint256){
        ...
        //Функция для обмена второго токена на первый, принимает количество второго токена возвращает количество первого токена которое нужно получить
            function tradeSecondToFirst(uint256 secondAmount,address sender) public  returns(uint256) {
    ...
    //Функция для получения резерва второго токена возвращает количество второго токена в пуле 
        ...
        function getFirstToken() external view returns (address) {
        return address(FirstToken);
    }

    // Функция для получения адреса второго токена
    function getSecondToken() external view returns (address) {
        return address(SecondToken);
    }
    function tradeTokens(address fromToken, address toToken, uint256 amount, address sender) external returns (uint256) {
    if (fromToken == address(FirstToken) && toToken == address(SecondToken)) {
        return tradeFirstToSecond(amount, sender);
    } else if (fromToken == address(SecondToken) && toToken == address(FirstToken)) {
        return tradeSecondToFirst(amount, sender);
    } else {
